...
window.onload=function(){
  ...
  _canvas=document.getElementById('evCanvas');_ctx=_canvas.getContext('2d');
  _overCanvas=document.getElementById('evOverlayCanvas');_overCtx=_overCanvas.getContext('2d');
  _drawCanvas=document.getElementById('evDrawCanvas');_drawCtx=_drawCanvas.getContext('2d');
  _video=document.getElementById('evVideo');
  var bc=document.getElementById('evBannerCanvas');_bannerCtx=bc.getContext('2d');clearBannerCanvas();
...
function loadVideoBlob(f){evToast('Loading…');_video.src=URL.createObjectURL(f);_video.load();document.getElementById('evProjName').textContent=f.name.replace(/\.[^.]+$/,'');}
function onVideoLoaded(){
  ...
  evToast('Video loaded ✓','ok');setTimeout(bannerFromFrame,1000);
}
function enableButtons(on){['evExportBtn','evTrimBtn','evUploadBtn','mobExportBtn','mobTrimBtn','mobUploadBtn'].forEach(function(id){var el=document.getElementById(id);if(el)el.disabled=!on;});}
...
function renderFrame(){
  ...
  _vidLayers.forEach(function(vl){
    if(vl.video&&vl.video.readyState>=2){
      _ctx.save();_ctx.globalAlpha=(vl.opacity||100)/100;_ctx.drawImage(vl.video,vl.x,vl.y,vl.w,vl.h);_ctx.restore();
...
function wmPos(pos,w,h){var cW=_canvas.width,cH=_canvas.height,p=18;if(pos==='tl')return{x:p,y:p};if(pos==='tr')return{x:cW-w-p,y:p};if(pos==='bl')return{x:p,y:cH-h-p};if(pos==='br')return{x:cW-w-p,y:cH-h-p};return{x:cW/2-w/2,y:cH/2-h/2};}

// ── ZOOM ─────────────────────────────────────────────────────
function touchDist(t){var dx=t[0].clientX-t[1].clientX,dy=t[0].clientY-t[1].clientY;return Math.sqrt(dx*dx+dy*dy);}
function applyZoom(l,f){if(l.video){var b=layerBounds(l);l.w=Math.max(30,Math.round(b.w*f));l.h=Math.round(l.w*((l.video.videoHeight||9)/(l.video.videoWidth||16)));return;}if(l.type==='watermark'&&l.wmType==='image'){var b2=layerBounds(l);l.w=Math.max(20,Math.round(b2.w*f));l.h=Math.max(20,Math.round(b2.h*f));}else if(l.type==='watermark'){l.size=Math.max(20,Math.round((l.size||80)*f));}else if(l.type==='text'){l.fontSize=Math.max(8,Math.round((l.fontSize||36)*f));}else{l.size=Math.max(12,Math.round((l.size||64)*f));}}
...
function addWatermark(){if(!_video||!_video.src){evToast('Open a video first','err');return;}pushUndo();var type=document.getElementById('wmType').value;var l={id:'wm'+Date.now(),type:'watermark',wmType:type,text:type==='keytube'?'KEYTUBE':(document.getElementById('wmTxt').value||'@Channel'),position:document.getElementById('wmPos').value,size:parseInt(document.getElementById('wmSz').value)||80,opacity:parseInt(document.getElementById('wmOp').value)||70,color:'#ffffff',name:'🔖 Watermark'};if(type!=='text')loadWMImg(l);_layers.push(l);_sel=l;updLayers();updSelCtrl();evToast('Watermark added ✓','ok');}
...
function onMD(e){
  ...
  if(hit){_sel=hit;_drag={layer:hit,sx:p.x,sy:p.y,ox:hit.x||0,oy:hit.y||0};updSelCtrl();}else{_sel=null;updSelCtrl();}
}
function onMM(e){
  if(e.preventDefault)e.preventDefault();var p=getCP(e);
...
function setupTrimHandles(){
  var wrap=document.getElementById('trimWrap'),hl=document.getElementById('trimHandleL'),hr=document.getElementById('trimHandleR');if(!wrap||!hl||!hr)return;
  function bind(el,side){function start(e){e.preventDefault();function mv(e2){var r=wrap.getBoundingClientRect();var cx=e2.touches?e2.touches[0].clientX:e2.clientX;var pct=Math.max(0,Math.min(1,(cx-r.left)/r.width));var t=pct*(_video.duration||0);if(side==='l'){_trimStart=Math.min(t,_trimEnd-0.5);}else{_trimEnd=Math.max(t,_trimStart+0.5);}_trimStart=Math.max(0,_trimStart);_trimEnd=Math.min(_video.duration||0,_trimEnd);updateTrimUI();}function up(){document.removeEventListener('mousemove',mv);document.removeEventListener('mouseup',up);document.removeEventListener('touchmove',mv);document.removeEventListener('touchend',up);}document.addEventListener('mousemove',mv);document.addEventListener('mouseup',up);document.addEventListener('touchmove',mv,{passive:false});document.addEventListener('touchend',up);}el.addEventListener('mousedown',start);el.addEventListener('touchstart',start,{passive:false});}
  bind(hl,'l');bind(hr,'r');
}
function updateTrimUI(){
  ...
  if(reg){reg.style.left=lP+'%';reg.style.width=(rP-lP)+'%';}if(hl)hl.style.left=lP+'%';if(hr)hr.style.right=(100-rP)+'%';
  ...
  var si=document.getElementById('trimStartIn'),ei=document.getElementById('trimEndIn');if(si)si.value=_trimStart.toFixed(1);if(ei)ei.value=_trimEnd.toFixed(1);
  var badge=document.getElementById('evTrimBadge');if(badge){if(_trimActive){badge.classList.remove('hidden');badge.textContent='✂ Trim: '+fmtT(_trimStart)+' → '+fmtT(_trimEnd);}else badge.classList.add('hidden');}
}
...
function loadAudioBlob(f){evToast('Loading audio…');var reader=new FileReader();reader.onload=function(e){if(!_audioCtx)_audioCtx=new(window.AudioContext||window.webkitAudioContext)();_audioCtx.decodeAudioData(e.target.result,function(buf){_audioBuffer=buf;_audioTracks.push({name:f.name,buffer:buf,volume:0.8,duration:buf.duration});renderAudioTracks();evToast('Audio added ✓','ok');var agc=document.getElementById('audioGlobalCtrl');if(agc)agc.style.display='';var at=document.getElementById('tlAudioTrack');if(at)at.style.display='';},function(){evToast('Audio decode failed','err');});};reader.readAsArrayBuffer(f);}
function renderAudioTracks(){var list=document.getElementById('audioTracks'),empty=document.getElementById('audioEmpty');if(list)list.innerHTML='';if(!_audioTracks.length){if(empty)empty.style.display='block';return;}if(empty)empty.style.display='none';_audioTracks.forEach(function(t,i){var d=document.createElement('div');d.className='audio-track-item';d.innerHTML='<span style="font-size:1.1rem">🎵</span><div style="flex:1;min-width:0"><div style="font-size:.77rem;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+h(t.name)+'</div><div style="font-size:.68rem;color:var(--t2);margin-top:1px">'+fmtT(t.duration)+'</div></div><button onclick="removeAudio('+i+')" style="background:none;border:none;color:var(--t3);cursor:pointer;font-size:.8rem;padding:3px">✕</button>';if(list)list.appendChild(d);});var af=document.getElementById('tlAudioFill');if(af)af.style.width='100%';}
...
function addVideoLayerBlob(f){var vid=document.createElement('video');vid.src=URL.createObjectURL(f);vid.muted=true;vid.loop=true;vid.playsinline=true;vid.preload='auto';vid.onloadedmetadata=function(){var vl={id:'vl'+Date.now(),type:'vidlayer',name:f.name,video:vid,x:20,y:20,w:Math.round(_canvas.width*0.3),h:Math.round(_canvas.height*0.3),opacity:100};_vidLayers.push(vl);vid.play();renderVidLayers();evToast('Video layer added ✓','ok');};}
function renderVidLayers(){var list=document.getElementById('vidLayerList'),empty=document.getElementById('vidLayerEmpty');if(list)list.innerHTML='';if(!_vidLayers.length){if(empty)empty.style.display='block';return;}if(empty)empty.style.display='none';_vidLayers.forEach(function(vl,i){var d=document.createElement('div');d.style.cssText='padding:8px 10px;border-bottom:1px solid var(--brd)';d.innerHTML='<div style="display:flex;align-items:center;gap:7px;margin-bottom:5px"><span>🎞</span><span style="font-size:.77rem;font-weight:600;flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+h(vl.name)+'</span><button onclick="removeVidLayer('+i+')" style="background:none;border:none;color:var(--t3);cursor:pointer">✕</button></div><label style="font-size:.64rem;color:var(--t2);font-weight:700;text-transform:uppercase;display:block;margin-bottom:3px">Size</label><input type="range" class="ev-range" min="50" max="'+_canvas.width+'" value="'+vl.w+'" oninput="resizeVidLayer('+i+',this.value)" style="width:100%">';if(list)list.appendChild(d);});}
...
// ── EXPORT — frame-by-frame seeking (correct speed, no malform) ──
...
function exportProcess(){
  return new Promise(function(resolve,reject){
    ...
    if(!window.MediaRecorder){reject(new Error('Use Chrome or Edge for export'));return;}
    ...
    var fps    = 25;
    var fInt   = 1 / fps;
    var total  = Math.ceil((end - start) * fps);

    // Pick mime
    var mime = '';
    ['video/webm;codecs=vp8','video/webm'].forEach(function(m){if(!mime&&MediaRecorder.isTypeSupported(m))mime=m;});
    if(!mime){reject(new Error('No supported format — try Chrome'));return;}
    ...
    var stream;try{stream=off.captureStream(fps);}catch(e){reject(new Error('Canvas capture not supported'));return;}

    var rec;
    try{rec=new MediaRecorder(stream,{mimeType:mime,videoBitsPerSecond:2500000});}
    catch(e){reject(new Error('Recorder init failed: '+e.message));return;}

    var chunks=[],done=false;
    rec.ondataavailable=function(e){if(e.data&&e.data.size>0)chunks.push(e.data);};
    rec.onstop=function(){
      _video.removeEventListener('seeked',onSeeked);
      if(!chunks.length){reject(new Error('No frames captured'));return;}
      resolve(new Blob(chunks,{type:mime}));
    };

    // Pause video, start recorder, seek to first frame
    _video.pause();
    rec.start(100);

    var frameIdx=0;

    // Draw current frame onto offscreen canvas including all overlays
    function drawFrame(){
    ...
    function onSeeked(){
      drawFrame(); // draw this frame to offscreen canvas

      frameIdx++;

      // Hold exactly one frame duration so MediaRecorder captures it
      setTimeout(function(){
        if(_exportCancelFlag){done=true;rec.stop();reject(new Error('cancelled'));return;}
        if(frameIdx>=total){
          setExportProg(99,'Finalizing…');
          setTimeout(function(){if(!done){done=true;rec.stop();}},600);
          return;
        }
        seekToFrame(frameIdx);
      }, Math.round(1000/fps)); // 40ms at 25fps
    }

    function seekToFrame(idx){
      var t=start+(idx*fInt);
      t=Math.min(t,end-0.001);
      var pct=Math.round(idx/total*100);
      setExportProg(pct,'Exporting frame '+(idx+1)+' / '+total+'  ('+pct+'%)');
      _video.currentTime=t;
    }

    _video.addEventListener('seeked',onSeeked);
    seekToFrame(0); // kick off first frame
  });
}
...
function canvasToBlob(canvas,type,quality){return new Promise(function(resolve,reject){if(!canvas){reject(new Error('No canvas'));return;}try{canvas.toBlob(function(b){b?resolve(b):reject(new Error('toBlob returned null'));},type||'image/jpeg',quality||0.88);}catch(e){reject(e);}});}
...
async function doUpload(){
  ...
  try{
    ...
    if(!blob){
      ...
      catch(exportErr){document.getElementById('evExportOv').classList.remove('open');if(exportErr.message==='cancelled'){btn.disabled=false;prog.style.display='none';return;}throw new Error('Export failed: '+exportErr.message);}
      document.getElementById('evExportOv').classList.remove('open');openModal('uploadModal');
    }
    var sizeMB=blob.size/1024/1024;
    setUpProg('Export ready ('+sizeMB.toFixed(1)+' MB). Uploading banner…',8);
    // STEP 2 — Banner (optional)
    var coverURL='';
    try{var bannerBlob=await canvasToBlob(document.getElementById('evBannerCanvas'),'image/jpeg',0.88);setUpProg('Uploading banner…',18);var cRes=await uploadToCloudinary(new File([bannerBlob],'banner.jpg',{type:'image/jpeg'}),CDN_USER,null);coverURL=cRes.url;}catch(be){console.warn('Banner skipped:',be.message);}
    // STEP 3 — Size check
    ...
    var saved=await new Promise(function(res,rej){api('addMovie',{gmail:_user.gmail,name:title,description:(document.getElementById('upDesc').value||'').trim(),category:(document.getElementById('upCat')||{value:'comedy'}).value,type:(document.getElementById('upType')||{value:'movie'}).value,cover:coverURL,videoURL:videoURL,downloadURL:videoURL,year:new Date().getFullYear(),isNew:true,featured:false},function(r){if(r&&r.ok)res(r);else rej(new Error(r?r.msg||'Save failed':'No server response'));});});
    // SUCCESS
  ...
  }catch(e){btn.disabled=false;prog.style.display='none';err.textContent=e.message||'Upload failed.';evToast(e.message||'Upload failed','err');console.error('[doUpload]',e);}
}
