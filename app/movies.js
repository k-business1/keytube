...
function buildPlayer(url){
  ...
  if(/\.(mp4|webm|ogg|mkv|mov)(\?.*)?$/i.test(url))return '<div style="position:relative;width:100%;height:100%;background:#000;overflow:hidden"><video autoplay controlsList="nodownload" src="'+h(url)+'" style="width:100%;height:100%"></video><div class="v-time" style="position:absolute;top:15px;left:50%;transform:translateX(-50%);background:rgba(0,0,0,0.75);color:#fff;padding:6px 14px;border-radius:20px;font-family:sans-serif;font-size:13px;display:none;z-index:3;pointer-events:none">0:00 / 0:00</div><div style="position:absolute;top:0;left:0;width:100%;height:100%;z-index:2" oncontextmenu="return false;" ontouchstart="this._sx=event.touches[0].clientX;var v=this.closest('div').querySelector('video');this._st=v.currentTime;" ontouchmove="var dx=event.touches[0].clientX-this._sx;var v=this.closest('div').querySelector('video');var t=Math.max(0,Math.min(v.duration||0,this._st+(dx/15)));v.currentTime=t;var p=this.closest('div').querySelector('.v-time');p.style.display='block';p.innerText=Math.floor(t/60)+':'+('0'+Math.floor(t%60)).slice(-2)+' / '+Math.floor((v.duration||0)/60)+':'+('0'+Math.floor((v.duration||0)%60)).slice(-2);" ontouchend="this.closest('div').querySelector('.v-time').style.display='none';var dx=(event.changedTouches?event.changedTouches[0].clientX:0)-this._sx;if(Math.abs(dx)<5){var v=this.closest('div').querySelector('video');v.paused?v.play():v.pause();}" onclick="var v=this.closest('div').querySelector('video');v.paused?v.play():v.pause();"></div></div>';
